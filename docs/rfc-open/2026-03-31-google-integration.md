...
1. **Initiate** — `GET /api/v1/google/authorize`
   ...
   - Backend generates PKCE `code_verifier` and `code_challenge`, stores verifier in Valkey (keyed by state, TTL 10 min)
...
2. **Callback** — `GET /api/v1/google/callback`
   ...
   - Backend retrieves PKCE `code_verifier` from Valkey
...
**Concurrent refresh protection**: Under Falcon's fiber concurrency, multiple fibers may simultaneously detect an expired token and attempt refresh. Use optimistic locking to ensure exactly-one refresh:

```ruby
# In GoogleConnection model:
def refresh_token_if_needed!
  return unless token_expires_at < 5.minutes.from_now

  # Optimistic lock: only one fiber wins the UPDATE
  updated = GoogleConnection
    .where(id: id, updated_at: updated_at)  # version check
    .update_all(access_token_enc: new_token, token_expires_at: new_expiry, updated_at: Time.current)

  reload if updated > 0  # Winner reloads; losers retry and see fresh token
end
```
...
**Additional verification**: Use the `X-Goog-Channel-Token` header for cryptographic verification. When creating a watch channel, include a per-subscription HMAC token:

```ruby
token = OpenSSL::HMAC.hexdigest("SHA256", Rails.application.secret_key_base, sync_state.id)
# Include token when creating the watch channel
# Verify token on incoming notifications before processing
```

This prevents forged notifications if a channel_id is leaked or guessed.
...
`googleauth` handles token refresh via `Google::Auth::UserRefreshCredentials`. DailyWerk wraps this with custom token persistence (database instead of file/Valkey token store) and workspace-scoped credential resolution.
