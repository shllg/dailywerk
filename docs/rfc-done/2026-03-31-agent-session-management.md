...
- **Inline session rotation** in `Session.resolve` — automatic, invisible to the user
- **Cross-session context bridge** — summary inheritance + sliding window of recent summarized messages from the previous session

### What This RFC Does NOT Cover

- Tool system / ReAct loop (future RFC — `AgentRuntime` is designed to accept tools later)
- **Structured memory system** (MemoryRetrievalService, memory_entries) — Level 3 context continuity. See [PRD 03 §7](../prd/03-agentic-system.md#7-memory-architecture) and [PRD 07 Future Work: Cross-session memory](../prd/07-future-work.md). This RFC's summary + sliding window is designed to be replaced/enhanced by the memory system when it ships.
...
def self.resolve(agent:, gateway: "web")
  # ... existing create_or_find_by! logic ...

  if session.stale?
    previous_summary = session.summary
    session.archive!
    session = create_or_find_by!(...) do |s|
      s.summary = previous_summary  # Inherit context
...
# Staleness is configurable per agent:
def stale?
...
def inactivity_threshold
  hours = agent.params&.dig("session_timeout_hours") || 4
  hours.to_i.hours
end
```

**UX principle**: One agent = one chat. The user never sees a "new session" UI. Session rotation is invisible infrastructure. On the web UI, we can optionally show a subtle separator. On Telegram/Signal, the agent simply continues naturally — the summary + sliding window (D10) provide enough context that the agent doesn't seem confused.

### D10: Cross-session context bridge (summary + sliding window)

**Problem**: After session rotation, the agent has a summary of the previous conversation but no specific recent messages. If the user (especially on Telegram/Signal where old messages are always visible) references something from the last few messages of the previous session, the summary may not have captured those details.

**Solution**: A two-level cross-session context bridge:

**Level 1 — Summary inheritance** (always active): The previous session's compaction summary is copied to the new session's `summary` field. `ContextBuilder` injects it into the system prompt as "Previous Context". This provides the big picture.

**Level 2 — Sliding window of recent summarized messages** (for fresh sessions): When a new session has no messages yet (just created via rotation), `ContextBuilder` also includes the last N messages from the most recently archived session. These messages use **summarized content** (via `content_for_context` / `MessageSummarizer`) — not raw content — so a 50k-char paste from the previous session becomes a ~400-char summary in the new session's context.
...
def build
  ...
  bridge = bridge_messages_context
  prompt_parts << bridge if bridge.present?
...
# Returns summarized recent messages from the previous session,
# only when the current session is fresh (no messages yet).
# Uses content_for_context (which applies media_description)
# and MessageSummarizer (which condenses long messages) to
# prevent large messages from polluting the new context.
#
# @return [String, nil]
def bridge_messages_context
  return nil if @session.context_messages.any?

  previous = find_previous_session
  return nil unless previous

  msgs = previous.messages
  ...
  formatted = msgs.map do |m|
    text = MessageSummarizer.call(m.content_for_context, model: compaction_model)
    "[#{m.role}] #{text}"
  end
...
**Design note**: The sliding window uses `MessageSummarizer` so that long messages are condensed. A 50k-char user paste becomes ~400 chars. A short "yes, do it" stays verbatim. This prevents the previous session's large content from dominating the new session's context budget.

**Future replacement**: When the structured memory system ships (PRD 03 §7), it will provide richer cross-session context (facts, preferences, decisions as queryable entries). The sliding window can be retired or reduced in scope.
...
class Session < ApplicationRecord
  ...
  # acts_as_chat defines has_many :context_messages internally; we override
  # it below with a scoped version. Last definition wins in Rails.
  acts_as_chat messages: :context_messages, model_class: "RubyLLM::ModelRecord"

  belongs_to :agent

  # Override the association defined by acts_as_chat to exclude compacted messages.
  # ruby_llm's internal methods use `messages_association` which delegates to
  # `send(:context_messages)` — so this scoped version is what the LLM sees.
  # Note: dependent: :destroy is intentionally omitted here (handled by :messages).
  has_many :context_messages,
           -> { where(compacted: false).order(:created_at) },
           class_name: "Message",
           foreign_key: :session_id,
  ...
  # Unscoped association for cascade deletes and full message access.
  has_many :messages, dependent: :destroy, inverse_of: :session
  ...
  # If the active session is stale (inactive beyond the agent's timeout),
  # it is archived and a new session is created automatically.
  # The new session inherits the previous session's summary for context continuity.
  #
  # @param agent [Agent]
  ...
  def self.resolve(agent:, gateway: "web")
    ...
    # Inline rotation: if the session is stale, archive it and create a fresh one.
    # The new session inherits the summary so ContextBuilder can inject prior context.
    if session&.stale?
      previous_summary = session.summary
      session.archive!
      session = nil  # Force creation below
    end

    session || create_new_session(agent:, workspace:, gateway:,
                                  inherited_summary: previous_summary)
  end

  # Whether this session has been inactive beyond the agent's configured timeout.
  #
  # @return [Boolean]
  def stale?
    return false if last_activity_at.blank?
  ...
  # Inactivity duration before session rotation. Configurable per agent
  # via agent.params["session_timeout_hours"]. Defaults to 4 hours.
  #
  # @return [ActiveSupport::Duration]
  def inactivity_threshold
    hours = agent.params&.dig("session_timeout_hours") || DEFAULT_SESSION_TIMEOUT_HOURS
    hours.to_i.hours
  end
  ...
  # @return [Integer]
  def active_context_tokens
    last_msg = messages.where(compacted: false).order(:created_at).last
    return 0 unless last_msg
  ...
  def estimated_context_tokens
    total_chars = messages.where(compacted: false)
                         .sum("coalesce(length(content), 0)")
    (total_chars / 4.0).ceil
  end

  # Context window size for the session's model.
  # Uses the direct context_window column on ruby_llm_models.
  #
  # @return [Integer]
  ...
  def self.find_active_session(agent:, workspace:, gateway:)
    where(agent:, workspace:, gateway:, status: "active").first
  end
  private_class_method :find_active_session

  # @return [Session]
  def self.create_new_session(agent:, workspace:, gateway:, inherited_summary: nil)
    ...
    create_or_find_by!(
      ...
      session.summary = inherited_summary  # Cross-session context bridge
...
Assembles the full LLM context: system prompt (from `PromptBuilder`) + session summary + cross-session bridge messages + active messages.
...
#   2. Session summary (from prior compactions or inherited from previous session)
#   3. Bridge messages (summarized recent messages from previous session, if fresh)
#   4. Active messages (non-compacted, chronological — managed by acts_as_chat)
class ContextBuilder
  ...
  # Returns summarized recent messages from the previous session.
  # Only active for fresh sessions (no messages yet) after rotation.
  # Uses content_for_context (media descriptions) and MessageSummarizer
  # (condensed long messages) to prevent large content from polluting
  # the new session's context budget.
  #
  # @return [String, nil]
  def bridge_messages_context
    return nil if @session.context_messages.any?
...
# Cleans up sessions that have been inactive for more than 7 days.
#
# Note: Most session rotation happens inline in Session.resolve (D9)
# when the user returns after inactivity. This cron job catches
# truly abandoned sessions where the user never returned.
#
# Runs as a GoodJob cron job (daily). Does NOT include WorkspaceScopedJob —
# operates cross-workspace using Current.without_workspace_scoping.
class ArchiveStaleSessionsJob < ApplicationJob
  ...
  def perform
    ...
    # Must bypass WorkspaceScoped default_scope — this job has no workspace context.
    Current.without_workspace_scoping do
...
| Concurrent messages near threshold | GoodJob `perform_limit: 1` prevents overlapping compaction. Only one job runs per session. `total_limit: 2` prevents excessive queue buildup. |
| Context still exceeds limit after compaction | Deferred (TODO): Emergency truncation not implemented in MVP. Provider returns context-length error, ChatStreamJob broadcasts error to user. The 25% buffer handles normal usage. |
| Session rotation (inline) | `Session.resolve` checks `stale?` (default 4h inactivity). If stale: archive old session → create new → inherit summary. `create_or_find_by!` handles concurrent resolve race. Invisible to user on all gateways. |
| Cross-session context on Telegram/Signal | Bridge messages (D10): ContextBuilder includes last 10 summarized messages from previous session. Uses `MessageSummarizer` so 50k-char pastes become ~400-char summaries. Bridge only active for fresh sessions (no messages yet). |
| User references old detail after rotation | Summary + bridge messages provide best-effort context. If specific detail was lost in summarization, agent should acknowledge the gap naturally. Structured memory system (future) will improve this. |
